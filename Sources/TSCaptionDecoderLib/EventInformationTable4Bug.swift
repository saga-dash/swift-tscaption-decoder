// 
//  EventInformationTable4Bug.swift
//  TSCaptionDecoderLib
//
//  Created by saga-dash on 2018/09/03.
//
...
// g1,e1のP/Fで188Byteを超える場合に送出されない？現象に対応
public struct EventInformationTable4Bug {
    ...
    public init?(_ data: Data, _ _header: TransportPacket? = nil) throws {
        ...
        repeat {
            do {
                let event = try Event4Bug(wrapper)
                array.append(event)
...
extension EventInformationTable4Bug : CustomStringConvertible {
...
extension EventInformationTable4Bug {
    ...
    public init(_ wrapper: ByteArray) throws {
        ...
        repeat {
            ...
            do {
                ...
                try wrapper.setIndex(wrapper.getIndex() + wrapper.count)
                break
            }
        } while payloadLength > 0
...
extension Event4Bug : CustomStringConvertible {
    public var description: String {
